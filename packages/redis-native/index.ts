import { IAdapter, IMessage } from '../core/index'; // Relative import for monorepo structure
...
export class RedisStreamAdapter extends EventEmitter implements IAdapter {
  ...
  async publish(topic: string, payload: any): Promise<void> {
    ...
    await this.client.sendCommand(['XADD', topic, '*', 'data', JSON.stringify(payload)]);
  }
  ...
  private async startConsumerLoop() {
      ...
      while (this.isConsuming) {
          ...
          try {
              ...
              const res = await this.subClient.sendCommand(['XREAD', 'BLOCK', '1000', 'STREAMS', ...streams, ...ids]);
              if (res) {
                  this.processMessages(res);
              }
          } catch (err) {
              // Timeout returns null usually, but error handling is good
              if ((err as any)?.message !== 'null') { // check if custom timeout handling needed
          ...
          for (const messageData of messages) {
              ...
              lastId = id; // Update last seen ID
               ...
               for (let i=0; i<fields.length; i+=2) {
                   if (fields[i] === 'data') {
                       try { payload = JSON.parse(fields[i+1]); } catch(e) { payload = fields[i+1]; }
                   }
               }
